...
export default function CommunityPostsCard({ post, index, isCurrentUserProfile, handleDelete }) {
    ...
    return (
        <>
            <Card key={index} direction={'row'} className="w-full justify-center" >
                ...
                <NavLink to={`/posts/${post.id}`}><Heading size='md' className="hover:text-[#448960] w-full mb-[0.17em]">{post.title}</Heading></NavLink>
                    <Flex direction={'row'} className="justify-start items-center text-gray-500">
                        <MdLocationPin />
                ...
                {/* {!!isCurrentUserProfile &&
                    (
                        <ButtonGroup spacing='2' colorScheme='green' className="bottom-0 mt-[1em]">
                            <Button _hover={'none'} fontSize={40} onClick={() => handleDelete(post.id, post.user_id)} variant='ghost'> <FaTrashCan /> </Button>
                        </ButtonGroup>
                    )
                } */}
            </Card>
